...
int main(int argc, char* argv[])
{
    ...
    if (argc != 4)
    {
        printf("Usage: %s <number_of_threads> <total_time> <algorithm_number (0=bakery; 1=gate)>\n", argv[0]);
        return 3;
    }

    num_threads = atoi(argv[1]);
    param_total_time  = atoll(argv[2]);
    j = atoi(argv[3]); // o algoritmo é dado como um parâmetro de entrada

    // for (j = 1; j < num_algorithms; ++j)
    // {
    ...
    for (k = 0; k < NUM_EXECUTIONS; ++k)
    {
        ...
        // printf("nthreads, sc_time, total_time, avg_accesses, stddev_accesses\n");
        printf("%d, %d, %lu, ", num_threads, param_total_time, (time2-time1));
