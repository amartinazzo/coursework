...
run_for_if() {
    ...
    for num_ifs in $(seq 0 9); do
        ...
        # echo -n "$SIZE_VECTOR,$NUM_THREADS,$num_ifs," >> dataset.csv
        # ./temp $1 $2 >> dataset.csv

        tmp=$(./temp $1 $2)
        if [ ${tmp%.*} -eq ${max%.*} ] && [ ${tmp#*.} \> ${max#*.} ] || [ ${tmp%.*} -gt ${max%.*} ]; then
            max="$tmp"
            max_ifs="$num_ifs"
        fi
        if [ ${tmp%.*} -eq ${min%.*} ] && [ ${tmp#*.} \< ${min#*.} ] || [ ${tmp%.*} -lt ${min%.*} ]; then
            min="$tmp"
            min_ifs="$num_ifs"
        fi
    done
    echo "$max_ifs" >> dataset-max.csv
    echo "$min_ifs" >> dataset-min.csv
}
